...
async function migrateToWorkspaces() {
    ...
    try {
        ...
        try {
            ...
            const existingWorkspaces = await workspace_1.WorkspaceModel.countDocuments().session(session);
            ...
            let placeholderUser = await user_1.UserModel.findById(PLACEHOLDER_USER_ID).session(session);
            if (!placeholderUser) {
                ...
                const created = await user_1.UserModel.create([
                    {
                        ...
                        role: "student",
                        profileImage: "https://res.cloudinary.com/dgr7l5nsx/image/upload/w_100,h_100,c_fill/v1754899421/profile_pictures/wtvbkvjnxrbdzfvjcmbi.png",
                        workspaces: [],
                        teams: [],
                        assignedTeams: [],
                        schemaVersion: "v2",
            ...
            const oldestTeacher = await user_1.UserModel.findOne({ role: "teacher" })
                .sort({ _id: 1 })
                .session(session);
            if (!oldestTeacher) {
            ...
            const theAcademy = await workspace_1.WorkspaceModel.create([
                {
                    name: "The Academy",
            ...
            const allUsers = await user_1.UserModel.find({}).session(session);
            ...
            const allStudents = await user_1.UserModel.find({
                role: "student",
                _id: { $ne: PLACEHOLDER_USER_ID }, // Exclude placeholder user
            }).session(session);
            const allTeachers = await user_1.UserModel.find({ role: "teacher" }).session(session);
            const spring2025Team = await Team_1.TeamModel.create([
                {
            ...
            const allClasses = await Class_1.ClassModel.find({}).session(session);
            ...
            const projectUpdateResult = await Projects_1.Project.updateMany({ schemaVersion: { $exists: false } }, { $set: { schemaVersion: "v2" } }).session(session);
            ...
            const commentUpdateResult = await Comment_1.CommentModel.updateMany({ schemaVersion: { $exists: false } }, { $set: { schemaVersion: "v2" } }).session(session);
            ...
            const replyUpdateResult = await Reply_1.Reply.updateMany({ schemaVersion: { $exists: false } }, { $set: { schemaVersion: "v2" } }).session(session);
